...
`data/derived/source_quote_audit.csv` is hand-authored (not regenerated by `make derived`): the 25-row Gate 1 ledger that binds every manuscript forum/site bibliography key and every quote-bank entry to an approved quotation, context check, and claim boundary. About and Terms are separate rows so each bib key binds to exactly one URL. `make validate` requires the file and asserts its row count.
