...
| `paper/generated/*.tex` | `make tables` | no |
| `paper/figures/*.pdf` | `make figures` | no |
| `paper/graphical_abstract.{pdf,png}` | `make graphical-abstract` | no |
...
`make validate` and `make claims` skip manuscript bindings when `paper/` is absent, and enforce them when the tree is present.
...
PDF builds additionally require a TeX distribution with a working `latexmk`. MiKTeX needs Perl on `PATH` for `latexmk`; installing Strawberry Perl or using TeX Live avoids that dependency.
