...
The difficult subset contains 45 episodes from 14 threads. Each episode records its lifecycle stage, primary code, modifiers, evidence form, detectability, consequence, public artifact, the construct it is a counterexample to, its first post anchor where the thread exposes one, and coding confidence. An episode is a contiguous run of posts that either raises a new initiating problem or moves to a new lifecycle stage; two adjacent runs sharing both are one episode.

\section{Hard-case adjudication set}
\label{sec:s-adjudication}
All coding was performed by one analyst, so no inter-rater statistic is reported. The release instead publishes the prepared adjudication set in \path{data/derived/reliability_subset.csv}. It covers the same 14 threads that were segmented into episodes and records, for each thread, the expected primary code, the most plausible competing code, why disagreement is likely, the specific adjudication question, the exact number of episodes the segmentation is expected to yield, the coded read scope, and a priority. Twelve threads are marked critical and two high.
...
\input{generated/partial_score_sensitivity.tex}

\input{generated/association_leave_one_out.tex}

\input{generated/denominator_sensitivity.tex}

The partial-score analysis changes the numerical level of each rubric summary, as expected, but makes that dependence explicit. It does not create a basis for ranking IAS, RMC, PDC, and OC against one another. The leave-one-thread-out analysis retains B5--B6, B5--B8, and B2--B7 in the leading three positions across all 55 deletions. B4--B8 remains in the top five throughout. B8--B9 falls to rank six under two deletions and retains the pilot attention threshold under 16 of 55 deletions; it is therefore interpreted as exploratory. The denominator review preserves the declared primary units and reports nearby defensible alternatives. The largest conceptual sensitivity concerns B7: excluding failure and recovery policy yields a 12-field nominal core, weighted opening completeness of 58.3\%, and discovery of 7/7 incomplete core fields.
...
\input{generated/quotations.tex}

\section{Reproduction}
Run \code{make derived}, \code{make validate}, \code{make test}, \code{make paper}, and \code{make supplement}. Expected values are asserted in \code{tests/test\_published\_values.py} and \code{tests/test\_robustness.py}. Community records validate against JSON Schema Draft 2020-12.
