...
from .io import read_csv, read_csv_many
from .metrics import association_from_counts, context_expansion_ratio, wilson_interval


def compute_release_results(root: str | Path) -> dict[str, Any]:
    root_path = Path(root)
    evidence = read_csv_many(sorted((root_path / "data/derived").glob("evidence_register_part_*.csv")))
    episodes = read_csv_many(sorted((root_path / "data/derived").glob("episode_register_part_*.csv")))
    ...
    constraint_discovery = sum(row["Identified in discussion?"] == "Yes" for row in incomplete) / len(incomplete)
    constraint_resolution = sum(row["Resolved with scenario-specific value?"] == "Yes" for row in incomplete) / len(incomplete)
    ...
    partial_or_better = sum(row["Alignment class"] in {"Aligned", "Partial"} for row in b8) / len(b8)

    b9 = read_csv(root_path / "data/metrics/b9_context_expansion.csv")
    initial = sum(row["Origin"] == "Initial" for row in b9)
    core_added = sum(row["Origin"] == "Reply-added" and row["Core execution scope?"] == "Yes" for row in b9)
    broad_added = sum(row["Origin"] == "Reply-added" and row["Broader deployment scope?"] == "Yes" for row in b9)
    conservative_added = sum(
        row["Origin"] == "Reply-added" and row["Counted in conservative grouping?"] == "Yes" for row in b9
    )
    ...
    partial_public = sum(row["Actionable public resolution"] in {"Yes", "Partial"} for row in b10) / len(b10)
    private_migration = sum(row["Private migration"] in {"Yes", "Partial"} for row in b10) / len(b10)
    ...
    return {
        ...
        "metrics": {
            ...
            "scheduling_opening_weighted_completeness": fmean(opening_scores),
